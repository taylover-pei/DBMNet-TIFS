...
class DGFANet(nn.Module):
    ...
    def forward(self, input, parameters=None):
        out, out_cat = self.feature_generator(input)
        feature, cls_out = self.feature_embedder(out, parameters)
        depth_map = self.depth_estimator(out_cat)
        return feature, cls_out, depth_map
