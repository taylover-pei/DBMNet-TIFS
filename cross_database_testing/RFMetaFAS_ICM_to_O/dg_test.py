...
def test(test_dataloader, model, threshold):
    ...
    number = 0
    with torch.no_grad():
        for iter, (input, target, videoID, image_path) in enumerate(test_dataloader):
            input = Variable(input).cuda()
            target = Variable(torch.from_numpy(np.array(target)).long()).cuda()
            feature, cls_out, depth_map = model(input)
    ...
    return [valid_top1.avg, cur_EER_valid, cur_HTER_valid, auc_score, ACC_threshold]

def main():
    net = DGFANet().cuda()
    test_data = sample_frames(flag=3, num_frames=config.tgt_test_num_frames, dataset_name=config.tgt_data)
